...
writes:
  ...
  - STATUS.md
gate: none
signoff: bom-approval
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, AskUserQuestion
---

# select-parts —— 选材（流水线 ④）

> 能力声明见 frontmatter。**needs: multimodal** —— 解析配件表 PDF 只 lead/Claude lane 跑。
> **门**：人工批 BOM → signoff `bom-approval`（AskUserQuestion 确认后 `tools/signoff.py approve bom-approval`）。
...
5. **人工门（结构化签字）**：把 BOM 摊给用户，用 **AskUserQuestion** 取得批准（尤其"需额外采购"项与关键件额定）。批准后落签：
   ```
   python tools/signoff.py approve bom-approval --by lead --note "BOM 已批"
   ```
   未签字时 `board.py done select-parts` 会被拒。批准后更新 `STATUS.md`，提示可并行 `/power-design` 与 `/interconnect`。
