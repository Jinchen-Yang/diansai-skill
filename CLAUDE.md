...
| `firmware/` | 控制 lane |
| `vision/` | 算法 lane |
| `00~10*.md`、`资源链接总表.md`、`README.md`（KB） | 人工维护，**skill 只读不改** |
...
`00~10` 与 `资源链接总表` 是参考知识（引脚说明 / 赛事章程 / 开发流程分工 / 金奖经验），skill **用 Read 查阅，不复制进库、不当需求来源、不当器件库存**。候选器件来自**主办方当年配件表**（用户丢进 `inputs/partslist/`）。常用对照：
