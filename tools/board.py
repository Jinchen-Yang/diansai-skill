...
命令:
    ...
    python tools/board.py done <id> [--by LANE] 标完成 + 自动派生下游任务
...
# 流水线规范任务图（id -> 角色/类型/依赖/标题/提示）。任务在某个依赖完成时才被"派生"创建。
TASKS = {
    "read-problem":     dict(owner="lead", kind="skill",  deps=[],                          title="读题/审题",                hint="/read-problem"),
    "plan-solution":    dict(owner="lead", kind="skill",  deps=["read-problem"],            title="方案+需求(签字表)",        hint="/plan-solution"),
    "setup-env":        dict(owner="lead", kind="skill",  deps=["plan-solution"],           title="环境全包(三lane清单)",     hint="/setup-env"),
    "select-parts":     dict(owner="lead", kind="skill",  deps=["plan-solution"],           title="选材(配件表→BOM)",         hint="/select-parts"),
    "power-design":     dict(owner="lead", kind="skill",  deps=["select-parts"],            title="供电(按堵转算)",           hint="/power-design"),
    "interconnect":     dict(owner="lead", kind="skill",  deps=["power-design"],            title="接线图(pinmap→wiring)",    hint="/interconnect"),
    ...
    "firmware-scaffold":dict(owner="控制", kind="skill",  deps=["interconnect"],            title="固件骨架(SysConfig/工程)", hint="/firmware-scaffold"),
...
def cmd_list(lane):
    ...
    for t in rows:
        ...
        print(f"  [{t['id']}] {t['title']}  ·{t['owner']}· {k}  → {t['hint']}")
...
def main():
    ...
    if cmd == "init":
        ...
        if len(args) < 2:
            print("用法: board.py done <id> [--by LANE]"); sys.exit(2)
