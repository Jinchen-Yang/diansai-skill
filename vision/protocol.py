...
PROTOCOL_SIG = "7362b70aad5ada78"   # 两端一致性签名，须与 protocol.h 相同
...
FUNC_AIM_OFFSET = 0x05  # 末端瞄准像素偏差；found=0 表示链路在线但当前未找到目标
...
def aim_offset(dx, dy, found):
    """末端瞄准像素偏差；found=0 表示链路在线但当前未找到目标"""
    return build(FUNC_AIM_OFFSET, struct.pack("<hhB", dx, dy, found))
